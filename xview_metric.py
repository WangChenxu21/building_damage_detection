...
class PathHandler:
    def __init__(self, pred_dir: Path, targ_dir: Path, img_id: str, index: str, test_hold: str):
        """
        Args:
        ...
        self.lp = pred_dir / f"{test_hold}_localization_{img_id}_{index}_prediction.png"  # localization prediction
        self.dp = pred_dir / f"{test_hold}_damage_{img_id}_{index}_prediction.png"  # damage prediction
        self.lt = targ_dir / f"{test_hold}_localization_{img_id}_{index}_target.png"  # localization target
        self.dt = targ_dir / f"{test_hold}_damage_{img_id}_{index}_target.png"  # damage target
...
class XviewMetrics:
    ...
    def get_path_handlers(self):
        ...
        for path in self.targ_dir.glob('*.png'):
            test_hold, loc_dmg, a, img_id, index, target = path.name.rstrip('.png').split('_')
            img_id = a + "_" + img_id
            ...
            if loc_dmg == 'localization':  # localization or damage is fine here
                self.path_handlers.append(PathHandler(self.pred_dir, self.targ_dir, img_id, index, test_hold))
    ...
    @classmethod
    def compute_score(cls, pred_dir, targ_dir):
        """
        Args:
