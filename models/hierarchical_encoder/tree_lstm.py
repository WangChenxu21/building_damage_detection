...
class WeightedHierarchicalTreeLSTMEndtoEnd(nn.Module):
    def __init__(self,
        ...
        if direction == 'bi_direction' or direction == 'bottom_up':
            ...
            self.tree_lstms.append(self.bottom_up_lstm)        
        # parent2child
        if direction == 'bi_direction' or direction == 'top_down':
            self.top_down_lstm = WeightedTopDownTreeLSTMEndtoEnd(in_dim, mem_dim, num_nodes, out_matrix, device)
            self.tree_lstms.append(self.top_down_lstm)
    ...
    def forward(self, inputs):
        ...
        for i in self.hierarchical_label_dict[self.root.idx]:
            for tree_lstm in self.tree_lstms:
                tree_lstm(self.label_trees[i + 1], inputs)
